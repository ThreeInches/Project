...
<base href="<%=basePath%>">
<meta charset="UTF-8">
<title>帅辰工程设备租赁管理系统人事管理页面</title>
				...
				<table class="table table-bordered table-hover">
					<tbody class="text-center">
						...
						<c:forEach var="u" items="${allUser }">
							<tr>
								<td>${u.id }</td><!--id是结果集的列名  -->s
								<td>${u.uname }</td>
								<td>${u.upwd }</td>
								<td>${u.power }</td>
								<td><a href="handle/delete?id=${u.id}">delete</a></td>
								<td><a href="update">update</a></td>
							</tr>
						</c:forEach>
						
						<tr>
							<td colspan="6">欢迎${sessionScope.myuser.uname }</td>
						</tr>
						<tr>
